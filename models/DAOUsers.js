    ...
    updateUser(userEmail, new_data, callback) {
        this.pool.getConnection(function (err, connection) {
            if (err) { // Si la conexión a la BBDD falla, se muestra un mensaje de error
                ...
                let query = "UPDATE users SET username = ?, date_birth = ?";
                let values;
                if (new_data.image != null) { // Hay nueva imagen
                    query += ", image = ?"
                    values = [new_data.username, new_data.date_birth, new_data.image, userEmail];
                }
                else { // No hay nueva imagen
                    values = [new_data.username, new_data.date_birth, userEmail];
                }
                query += " WHERE email = ?";
                connection.query(
                    query, values,
